...
import { mockCreators, mockContent, getUserAccessPasses } from '../lib/mock-data';
...
export function Feed() {
  ...
  const supportedCreators = mockCreators.filter(c => accessPasses.includes(c.id));
  
  const feedContent = supportedCreators.flatMap(creator => {
        ...
        <section className="flex-1 flex items-center justify-center px-4 py-12">
          <Card className="max-w-md w-full text-center">
            <CardContent className="pt-6 space-y-4">
              ...
              <div>
                <h3 className="font-semibold text-lg mb-2">No supported creators yet</h3>
    ...
    <div className="flex flex-col min-h-screen">
      <section className="py-12 px-4 border-b bg-muted/30">
        <div className="container mx-auto">
          <h1 className="text-3xl md:text-4xl font-bold mb-2">My Feed</h1>
          <p className="text-lg text-muted-foreground">
            Content from {supportedCreators.length} creator{supportedCreators.length !== 1 ? 's' : ''} you support
          </p>
        </div>
      </section>
